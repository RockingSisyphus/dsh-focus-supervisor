...
const taskKeys=['id','status','session_id','start_at','end_at','revision','check_interval_seconds'];
...
export function compactResult(name,value,args={}) {
  ...
  if(name==='focus_status')return {...pick(value,['running','shutting_down','last_sample_at','presence','capture_error','delivery_error','poll_error']),
    live:(value.live||[]).map(taskSummary),reports:(value.reports||[]).map(r=>pick(r,['id','task_id','phase','status','created_at','delivered']))};
