...
已知待处理：Windows 偶发唤回慢后备（BUG-27）、偶发报告停顿（BUG-32）；不影响本项目如实保留失败记录，参见 [当前问题](https://github.com/RockingSisyphus/dsh-focus-supervisor/blob/main/docs/known-issues.md)。
...
测试统一入口为 `dshmonitor-test-pack/run.py`，真实产品用例使用独立 VM、真实 DSH、正式后台、固定模型回复和实际 UI 操作；测试环境需自行配置，不会把普通电脑当作可清理的新机 VM。参见 [测试说明](https://github.com/RockingSisyphus/dsh-focus-supervisor/blob/main/dshmonitor-test-pack/README.md)、[架构](https://github.com/RockingSisyphus/dsh-focus-supervisor/blob/main/docs/architecture.md)、[使用详情](https://github.com/RockingSisyphus/dsh-focus-supervisor/blob/main/docs/usage.md)。
