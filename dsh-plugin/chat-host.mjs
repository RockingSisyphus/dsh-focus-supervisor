...
export async function apply(ctx,config={}) {
  ...
  tool('focus_act','统一执行提醒或窗口动作，由你根据情境选择。动手前应先用 focus_check 获取最新窗口数据，并用它的 report_id 与 target_ref。minimize_window 把目标窗口最小化；force_close 按 target_kind 关闭进程、窗口或浏览器标签。标签精确关闭失败时不自动扩大范围；回执会提示再次提醒用户。若用户仍未回到任务，显式传 force_kill=true 才会结束所属浏览器进程及全部窗口，可能连 DSH 页面一起关闭。窗口关闭保留进程后备。remind 自动同时弹窗、提示音和系统通知；返回实际结果。',{task_id:str('任务编号'),action:{type:'string',required:true,enum:['remind','minimize_window','force_close']},target_kind:{type:'string',enum:['process','window','browser_tab'],description:'force_close 的目标类型，默认 process'},force_kill:{type:'boolean',description:'仅用于 browser_tab；精确关闭失败后显式允许结束所属浏览器进程及其全部窗口，调用前应再次提醒用户'},message:str('要对用户说的话；remind 必填',false),image:{type:'string',enum:['question','gentle','warning','urgent','start','celebrate','sleep','scheduled','watching','checking']},report_id:str('窗口动作所依据的新报告',false),target_ref:str('报告中的窗口证据编号',false)},(a,e)=>request('/act',{...a,chat_url:`http://127.0.0.1:${ctx.webServer.port}/focus/open?session=${encodeURIComponent(session(e))}`,chat_registry:chatRegistry,session_id:session(e),action_id:createHash('sha256').update(session(e)+':'+e.callId).digest('hex')}));
  tool('focus_settings','读取或修改提示词、形象、采样和调试保留设置；AI 可与用户商量后修改严苛任务期间的设置。',{operation:{type:'string',required:true,enum:['get','update']},task_id:str('修改 task_prompt 时填写任务编号',false),patch_json:str('JSON 对象，可含 instructions、instructions_full、heartbeat_prompt、strict_heartbeat_prompt、task_prompt、mascot_size、away_heartbeats、sampling、reporting、debug_mode',false)},async(a,e)=>{if(a.operation==='get')return (await state()).settings||configuredPrompts();await wake();const result=await request('/settings/ai',{task_id:a.task_id,patch:JSON.parse(a.patch_json||'{}'),session_id:session(e)});return result;});
