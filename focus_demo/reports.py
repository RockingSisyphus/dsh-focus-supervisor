...
def browser_summary(effective):
    ...
    return {"snapshots": len(records), "successful_snapshots": sum("snapshot" in r for r in records),
            ...
            "pages": [{"title": clip(r.get("title", ""), 120), "captured_at": r.get("captured_at"),
...
def overview(effective, limit=12):  # 功能：统计覆盖整个检查区间，并提供可翻页的程序目录。
    ...
    return {"activity_changes": [{k:v for k,v in row.items() if k not in ("changes","evidence_refs","last_state")} | {"change_count":len(row["changes"]),"recent_changes":[c for c in row["changes"] if c["type"]!="observed"][-3:],"latest_evidence":row["evidence_refs"][-1:] } for row in effective.get("activity_changes",[])[:12]], "browser_semantics": browser_summary(effective), "desktop_screenshot": {k: v for k, v in (effective.get("desktop_screenshot") or {}).items() if k != "path"}, "gui_window_count": len(effective.get("window_inventory", [])), "programs": rows[:limit], "program_count": len(rows), "next_program_offset": limit if len(rows) > limit else None, "segment_count": len(effective.get("segments", [])), "effective_observed_seconds": effective.get("effective_observed_seconds", 0), "unobserved_gap_seconds": effective.get("unobserved_gap_seconds", 0), "catalog_note": "统计覆盖全部片段；程序目录可用 list_programs 翻页；同时可见的程序时长不能相加当作总工作时间。", "current_objects": [{k: o.get(k) for k in ("ref", "id", "app", "title", "focused", "visible", "visibility_note")} for o in current[:12]], "current_object_count": len(current), "current_objects_omitted": max(0, len(current)-12)}  # 当前对象只是提示，不替代完整目录。
