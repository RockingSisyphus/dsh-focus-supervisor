...
def test_settings_persist_and_keep_existing_permissions(tmp_path):
    ...
    core.configure({'patch':{'instructions':'CUSTOM','protect_task_changes':True,'sampling':{'text_chars':900}}},actor='ui')
    task=core.plan(dict(task_id='t',task_prompt='observe',agreement='work',project_dir=str(tmp_path),start_at=time.time()-1,end_at=time.time()+300,allow_early_finish=True,deadline_policy='discuss'),'chat')
